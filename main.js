...
var FolderSuggest = class extends import_obsidian.AbstractInputSuggest {
  ...
  getSuggestions(query) {
    ...
    return this.app.vault.getAllLoadedFiles().filter((f) => f instanceof import_obsidian.TFolder && f.path.toLowerCase().contains(lower));
  }
...
var SafiSiteAuditSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian.Setting(containerEl).setName("Audit folder").setDesc("Vault folder where audit notes are saved and listed in the dashboard.").addText((text4) => {
      text4.setPlaceholder("Site Audits").setValue(this.plugin.settings.auditFolder).onChange(async (value) => {
    ...
    new import_obsidian.Setting(containerEl).setName("PageSpeed Insights API key").setDesc("Optional. Enables Core Web Vitals and accessibility checks via Google PSI.").addText((text4) => {
      text4.inputEl.type = "password";
      text4.setPlaceholder("AIza\u2026").setValue(this.plugin.settings.psiKey).onChange(async (value) => {
        this.plugin.settings.psiKey = value.trim();
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian.Setting(containerEl).setName("User agent").setDesc("Optional override for the crawler's User-Agent header.").addText(
      (text4) => text4.setPlaceholder("SafiStudioScanner/0.1").setValue(this.plugin.settings.userAgent).onChange(async (value) => {
...
function hydration_attribute_changed(attribute, html4, value) {
  if (dev_fallback_default) {
    console.warn(`%c[svelte] hydration_attribute_changed
%cThe \`${attribute}\` attribute on \`${html4}\` changed its value between server and client renders. The client value, \`${value}\`, will be ignored in favour of the server value
  ...
  while (true) {
    if (node.nodeType === COMMENT_NODE) {
      ...
      if (data2 === HYDRATION_END) {
        ...
        depth -= 1;
      } else if (data2 === HYDRATION_START || data2 === HYDRATION_START_ELSE || // "[1", "[2", etc. for if blocks
      data2[0] === "[" && !isNaN(Number(data2.slice(1)))) {
        depth += 1;
      }
    }
    var next3 = (
...
function flatten(blockers, sync, async2, fn) {
  ...
  function run3() {
    Promise.all(async2.map((expression) => async_derived(expression))).then(finish).catch((error) => invoke_error_boundary(error, parent2)).finally(decrement_pending);
  }
  if (blocker_promise) {
...
function execute_derived(derived2) {
  ...
  if (!is_destroying_effect && parent2 !== null && derived2.v !== UNINITIALIZED && // if it was never evaluated before, it's guaranteed to fail downstream, so we try to execute instead
  (parent2.f & (DESTROYED | INERT)) !== 0) {
    derived_inert();
    return derived2.v;
  }
  set_active_effect(parent2);
  if (dev_fallback_default) {
...
function proxy(value) {
  ...
  function update_path(new_path) {
    ...
    for (const [prop3, source2] of sources) {
      tag(source2, get_label(path, prop3));
    }
    updating = false;
  }
  return new Proxy(
    ...
    value,
    {
      defineProperty(_, prop3, descriptor) {
        ...
        var s = sources.get(prop3);
        if (s === void 0) {
          with_parent(() => {
            var s2 = state(descriptor.value, stack2);
            sources.set(prop3, s2);
            if (dev_fallback_default && typeof prop3 === "string") {
      ...
      get(target, prop3, receiver) {
        if (prop3 === STATE_SYMBOL) {
          return value;
        }
        if (dev_fallback_default && prop3 === PROXY_PATH_SYMBOL) {
          return update_path;
        }
        var s = sources.get(prop3);
      ...
      has(target, prop3) {
        ...
        var s = sources.get(prop3);
        var has2 = s !== void 0 && s.v !== UNINITIALIZED || Reflect.has(target, prop3);
      ...
      set(target, prop3, value2, receiver) {
        var s = sources.get(prop3);
        var has2 = prop3 in target;
        if (is_proxied_array && prop3 === "length") {
        ...
        if (s === void 0) {
          if (!has2 || get_descriptor(target, prop3)?.writable) {
            s = with_parent(() => state(void 0, stack2));
            if (dev_fallback_default) {
              tag(s, get_label(path, prop3));
            }
            set(s, proxy(value2));
            sources.set(prop3, s);
          }
        ...
        if (!has2) {
          if (is_proxied_array && typeof prop3 === "string") {
            var ls = (
              /** @type {Source<number>} */
              sources.get("length")
            );
...
function get_label(path, prop3) {
  if (typeof prop3 === "symbol") return `${path}[Symbol(${prop3.description ?? ""})]`;
  if (regex_is_valid_identifier.test(prop3)) return `${path}.${prop3}`;
  return /^\d+$/.test(prop3) ? `${path}[${prop3}]` : `${path}['${prop3}']`;
}
...
function inspectable_array(array) {
  return new Proxy(array, {
    get(target, prop3, receiver) {
      var value = Reflect.get(target, prop3, receiver);
...
function child(node, is_text) {
  ...
  if (child2 === null) {
    ...
    var text4 = create_text();
    child2?.before(text4);
    set_hydrate_node(text4);
    return text4;
  }
  if (is_text) {
    merge_text_nodes(
  ...
  if (is_text) {
    if (next_sibling?.nodeType !== TEXT_NODE) {
      ...
      if (next_sibling === null) {
        last_sibling?.after(text4);
      } else {
        next_sibling.before(text4);
      }
      set_hydrate_node(text4);
      return text4;
    }
...
function create_element(tag2, namespace, is5) {
  if (namespace == null || namespace === NAMESPACE_HTML) {
    return (
      /** @type {T extends keyof HTMLElementTagNameMap ? HTMLElementTagNameMap[T] : Element} */
      is5 ? document.createElement(tag2, { is: is5 }) : document.createElement(tag2)
    );
  }
  return (
    /** @type {T extends keyof HTMLElementTagNameMap ? HTMLElementTagNameMap[T] : Element} */
    is5 ? document.createElementNS(namespace, tag2, { is: is5 }) : document.createElementNS(namespace, tag2)
  );
}
function merge_text_nodes(text4) {
  if (
    /** @type {string} */
    text4.nodeValue.length < 65536
  ) {
    return;
  }
...
function create_effect(type, fn) {
  ...
  if (dev_fallback_default) {
    while (parent2 !== null && (parent2.f & EAGER_EFFECT) !== 0) {
...
function create_trusted_html(html4) {
  return (
    /** @type {string} */
    policy?.createHTML(html4) ?? html4
  );
}
function create_fragment_from_html(html4) {
  var elem = create_element("template");
  elem.innerHTML = create_trusted_html(html4.replaceAll("<!>", "<!---->"));
  return elem.content;
}

// node_modules/svelte/src/internal/client/dom/template.js
function assign_nodes(start, end2) {
  var effect2 = (
    /** @type {Effect} */
    active_effect
  );
  if (effect2.nodes === null) {
    effect2.nodes = { start, end: end2, a: null, t: null };
  }
}
// @__NO_SIDE_EFFECTS__
...
function set_text(text4, value) {
  ...
  if (str !== /** @type {any} */
  (text4[TEXT_CACHE] ??= text4.nodeValue)) {
    text4[TEXT_CACHE] = str;
    text4.nodeValue = `${str}`;
  }
}
...
function if_block(node, fn, elseif = false) {
  ...
  function update_branch(key2, fn2) {
    if (hydrating) {
      ...
      if (key2 !== parseInt(data2.substring(1))) {
  ...
  listen_to_event_and_reset_event(input, "input", async (is_reset) => {
    ...
    if (value !== (value = get4())) {
      ...
      if (end2 !== null) {
        ...
        if (start === end2 && end2 === length && new_length > length) {
...
var Svelte4Component = class {
  ...
  constructor(options) {
    ...
    const props = new Proxy(
      ...
      {
        get(target, prop3) {
          return get2(sources.get(prop3) ?? add_source(prop3, Reflect.get(target, prop3)));
        },
        has(target, prop3) {
          if (prop3 === LEGACY_PROPS) return true;
          get2(sources.get(prop3) ?? add_source(prop3, Reflect.get(target, prop3)));
          return Reflect.has(target, prop3);
        },
        set(target, prop3, value) {
          set(sources.get(prop3) ?? add_source(prop3, value), value);
          return Reflect.set(target, prop3, value);
        }
      }
    );
...
function get_custom_element_value(prop3, value, props_definition, transform) {
  const type = props_definition[prop3]?.type;
  value = type === "Boolean" && typeof value !== "boolean" ? value != null : value;
  if (!transform || !props_definition[prop3]) {
    return value;
...
// node_modules/safi-studio-scanner/dist/http.js
...
function createLinkChecker(opts, pool) {
  const cache = /* @__PURE__ */ new Map();
  return function checkUrl(url) {
...
async function fetchRobots(origin, opts) {
  ...
  if (exists) {
    for (const line of res.html.split("\n")) {
      ...
      if (m)
        sitemaps.push(m[1].trim());
    }
  }
  return { exists, status: res.status, content: exists ? res.html : "", sitemaps };
}
async function fetchSitemap(origin, declared, opts) {
...
var DomHandler = class {
  ...
  onopentag(name, attribs) {
    ...
    const element2 = new Element2(name, attribs, void 0, type);
...
function appendChild(parent2, child2) {
  removeElement(child2);
  child2.next = null;
  child2.parent = parent2;
  if (parent2.children.push(child2) > 1) {
    const sibling2 = parent2.children[parent2.children.length - 2];
...
function prependChild(parent2, child2) {
  ...
  child2.parent = parent2;
  child2.prev = null;
  if (parent2.children.unshift(child2) !== 1) {
    const sibling2 = parent2.children[1];
...
function addConditionally(obj, prop3, tagName, where, recurse = false) {
...
function determineBranch2(decodeTree, current, nodeIndex, char) {
  ...
  while (lo <= hi) {
    const mid = lo + hi >>> 1;
    const slot2 = mid >> 1;
    const packed = decodeTree[nodeIndex + slot2];
  ...
  emitOpenTag(name) {
    ...
    if (impliesClose) {
      while (this.stack.length > 0 && impliesClose.has(this.stack[0])) {
        ...
        (_b = (_a5 = this.cbs).onclosetag) === null || _b === void 0 ? void 0 : _b.call(_a5, element2, true);
      }
    }
...
var hasOwn = (
  ...
  (_a3 = Object.hasOwn) !== null && _a3 !== void 0 ? _a3 : (object, prop3) => Object.prototype.hasOwnProperty.call(object, prop3)
);
var rspace = /\s+/;
  ...
  if (typeof name === "string" && value === void 0) {
    ...
    switch (name) {
      ...
      case "src": {
        if (!isTag2(el))
          return void 0;
        const prop3 = (_a5 = el.attribs) === null || _a5 === void 0 ? void 0 : _a5[name];
        if (typeof URL !== "undefined" && (name === "href" && (el.tagName === "a" || el.tagName === "link") || name === "src" && (el.tagName === "img" || el.tagName === "iframe" || el.tagName === "audio" || el.tagName === "video" || el.tagName === "source")) && prop3 !== void 0 && this.options.baseURI) {
...
var pseudos = {
  ...
  "only-of-type"(elem, { adapter: adapter2, equals: equals3 }) {
    const elemName = adapter2.getName(elem);
    return adapter2.getSiblings(elem).every((sibling2) => equals3(elem, sibling2) || !adapter2.isTag(sibling2) || adapter2.getName(sibling2) !== elemName);
  },
  "only-child"(elem, { adapter: adapter2, equals: equals3 }) {
    return adapter2.getSiblings(elem).every((sibling2) => equals3(elem, sibling2) || !adapter2.isTag(sibling2));
  }
};
  ...
  for (const attr3 of treeAdapter.getAttrList(node)) {
    ...
    if (attr3.namespace) {
      switch (attr3.namespace) {
        ...
        case NS.XLINK: {
          html4 += `xlink:${attr3.name}`;
          break;
        }
        default: {
          html4 += `${attr3.prefix}:${attr3.name}`;
        }
      }
    } else {
      html4 += attr3.name;
    }
    html4 += `="${escapeAttribute2(attr3.value)}"`;
  }
  return html4;
}
...
function buildPageContext(raw, origin, depth) {
  const $2 = load(raw.html || "");
  ...
  $2("img").each((_, el) => {
    ...
    images.push({
      ...
      width: $2(el).attr("width") ?? null,
      height: $2(el).attr("height") ?? null,
      loading: $2(el).attr("loading") ?? null
    });
  });
  return {
    url: raw.url,
...
// node_modules/safi-studio-scanner/dist/crawl.js
...
async function crawl(startUrl, opts, pool) {
  const origin = new URL(startUrl).origin;
  const seen = /* @__PURE__ */ new Set([normalize(startUrl)]);
...
// node_modules/safi-studio-scanner/dist/providers/playwright.js
var CWV_INIT = `
...
async function loadDeps() {
  try {
    ...
    throw new Error('Browser mode needs the optional "playwright" and "@axe-core/playwright" packages. Install them with: npm i playwright @axe-core/playwright && npx playwright install chromium');
  }
}
async function analyzePage(browser, AxeBuilder, url, opts) {
  ...
  try {
    ...
    return { ok: false, error: err.message, axe: [], axePasses: [], metrics: null };
  } finally {
    await context.close();
  }
}
async function renderPages(urls, opts) {
  ...
  try {
    ...
    throw new Error(`Could not launch Chromium. Run "npx playwright install chromium" first.
${err.message}`);
  }
  try {
    const pool = createPool(opts.concurrency);
    await Promise.all(urls.map((url) => pool(async () => {
...
// node_modules/safi-studio-scanner/dist/providers/psi.js
var ENDPOINT = "https://www.googleapis.com/pagespeedonline/v5/runPagespeed";
function metricsFrom(audits) {
  const num = (id) => Math.round(audits[id]?.numericValue ?? 0);
  return {
...
function accessibilityFrom(res) {
...
async function analyze(url, opts) {
  ...
  if (opts.psiKey)
    params.set("key", opts.psiKey);
  try {
    const res = await fetch(`${ENDPOINT}?${params.toString()}`, {
      signal: AbortSignal.timeout(Math.max(opts.timeout, 6e4))
    });
    if (!res.ok) {
      return { ok: false, error: `PSI HTTP ${res.status}`, axe: [], axePasses: [], metrics: null };
    }
    const data2 = await res.json();
    const audits = data2.lighthouseResult?.audits ?? {};
    const { axe, axePasses } = accessibilityFrom(data2);
    return { ok: true, axe, axePasses, metrics: metricsFrom(audits) };
  } catch (err) {
    return { ok: false, error: err.message, axe: [], axePasses: [], metrics: null };
  }
}
async function pageSpeedInsights(urls, opts) {
  const targets = urls.slice(0, opts.psiMaxPages);
  const out = /* @__PURE__ */ new Map();
  const pool = createPool(Math.min(opts.concurrency, 4));
  await Promise.all(targets.map((url) => pool(async () => {
    out.set(url, await analyze(url, opts));
  })));
  return out;
}

// node_modules/safi-studio-scanner/dist/runner.js
var SEVERITY_WEIGHT = { error: 10, warning: 4, info: 1 };
var WARN_CREDIT = 0.25;
async function runPage(page, site, rules, checkUrl) {
  const ctx = { page, site, checkUrl };
  const findings = [];
  for (const rule of rules) {
    if (rule.requiresBrowser && !page.browser)
      continue;
    let results;
    try {
...
function aggregate(startUrl, generatedAt, pages) {
  ...
  const categories = [...byCat.entries()].map(([category, fs]) => ({
    category,
...
var OVERALL_WEIGHT = { error: 22, warning: 11, info: 1 };
var WARN_FACTOR = 0.55;
...
// node_modules/safi-studio-scanner/dist/rules/core-seo.js
var coreSeoRules = [
  {
    ...
    run({ page }) {
      ...
      return [
        t ? { status: "pass", message: "Title present" } : { status: "fail", message: "Missing <title>" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      const t = page.$("title").first().text().trim();
      if (!t)
        return [{ status: "warn", message: "No title to measure" }];
      if (t.length < 30)
  ...
  },
  {
    ...
    run({ page }) {
      ...
      return [
        d ? { status: "pass", message: "Meta description present" } : { status: "fail", message: "Missing meta description" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      const d = page.$('meta[name="description"]').attr("content")?.trim() || "";
      if (!d)
        return [{ status: "warn", message: "No description to measure" }];
      if (d.length < 70)
        return [{ status: "warn", message: `Description short (${d.length} chars)` }];
      if (d.length > 160)
        return [{ status: "warn", message: `Description long (${d.length} chars)` }];
  ...
  },
  {
    ...
    run({ page }) {
      ...
      if (!c)
        return [{ status: "warn", message: "No canonical link" }];
      try {
  ...
  },
  {
    ...
    run({ page }) {
      const has2 = page.$("meta[charset]").length > 0 || /charset=/i.test(page.headers["content-type"] || "");
      return [
        has2 ? { status: "pass", message: "Charset declared" } : { status: "warn", message: "No charset declared" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      ...
      return [
        v ? { status: "pass", message: "Viewport present" } : { status: "fail", message: "Missing viewport meta" }
      ];
    }
  },
  {
...
// node_modules/safi-studio-scanner/dist/rules/content.js
var contentRules = [
  {
  ...
  },
  {
    ...
    run({ page }) {
      const text4 = page.$("body").text().replace(/\s+/g, " ").trim();
      const words = text4 ? text4.split(" ").length : 0;
      return [
        words < 300 ? { status: "warn", message: `Thin content (${words} words)` } : { status: "pass", message: `${words} words` }
      ];
    }
  },
  {
    ...
    run({ page }) {
      let empty2 = 0;
      page.$("h1,h2,h3,h4,h5,h6").each((_, el) => {
        if (!page.$(el).text().trim())
          empty2++;
      });
      return [
        empty2 === 0 ? { status: "pass", message: "No empty headings" } : { status: "warn", message: `${empty2} empty heading(s)` }
      ];
    }
  },
  {
    ...
    run({ page }) {
      ...
      return [
        density > 0.05 ? { status: "warn", message: `"${top}" is ${(density * 100).toFixed(1)}% of words` } : { status: "pass", message: "No keyword over-repetition" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      const text4 = page.$("body").text().replace(/\s+/g, " ").trim().length;
      const html4 = page.html.length || 1;
      const ratio = text4 / html4;
      return [
        ratio < 0.1 ? { status: "warn", message: `Low text ratio (${(ratio * 100).toFixed(1)}%)` } : { status: "pass", message: `Text ratio ${(ratio * 100).toFixed(1)}%` }
      ];
    }
  },
  {
    ...
    run({ page }) {
      ...
      return [
        lang ? { status: "pass", message: `lang="${lang}"` } : { status: "warn", message: "Missing lang on <html>" }
      ];
    }
  }
];

// node_modules/safi-studio-scanner/dist/rules/links.js
var linksRules = [
  {
  ...
  },
  {
    ...
    run({ page }) {
      return [
        page.redirectChain > 1 ? { status: "warn", message: `Page redirected ${page.redirectChain} times before loading` } : { status: "pass", message: "No redirect chain" }
      ];
    }
  },
  {
...
// node_modules/safi-studio-scanner/dist/rules/images.js
var imagesRules = [
  {
    id: "images/img-alt",
    category: "images",
    ...
    run({ page }) {
      ...
      return [
        missing.length === 0 ? { status: "pass", message: `All ${page.images.length} images have alt` } : {
          status: "fail",
  ...
  },
  {
    ...
    run({ page }) {
      ...
      return [
        missing.length === 0 ? { status: "pass", message: "All images have width and height" } : { status: "warn", message: `${missing.length} image(s) missing width/height` }
      ];
    }
  },
  {
    ...
    run({ page }) {
      ...
      return [
        legacy.length === 0 ? { status: "pass", message: "No legacy image formats" } : { status: "warn", message: `${legacy.length} image(s) not using WebP/AVIF` }
      ];
    }
  },
  {
    ...
    run({ page }) {
      ...
      const eager2 = page.images.slice(1).filter((i) => i.loading !== "lazy");
      return [
        eager2.length === 0 ? { status: "pass", message: "Below-the-fold images lazy loaded" } : { status: "warn", message: `${eager2.length} image(s) without loading="lazy"` }
      ];
    }
  },
  {
    ...
    run({ page }) {
      const bad = page.images.filter((i) => /\/(img|image|dsc|screenshot)?[-_]?\d{3,}\.\w+$/i.test(i.absUrl));
      return [
        bad.length === 0 ? { status: "pass", message: "Descriptive image filenames" } : { status: "warn", message: `${bad.length} image(s) with non-descriptive filenames` }
      ];
    }
  },
  {
    ...
    run({ page }) {
      const empty2 = page.images.filter((i) => !i.src.trim());
      return [
        empty2.length === 0 ? { status: "pass", message: "All images have src" } : { status: "fail", message: `${empty2.length} image(s) with empty src` }
      ];
    }
  }
];

// node_modules/safi-studio-scanner/dist/rules/structured-data.js
var structuredDataRules = [
  {
    ...
    run({ page }) {
      const has2 = /"@type"\s*:\s*"(Organization|WebSite|LocalBusiness)"/.test(page.html);
      return [
        has2 ? { status: "pass", message: "Organization or WebSite schema present" } : { status: "warn", message: "No Organization or WebSite schema" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      ...
      if (blocks.length === 0)
        return [{ status: "pass", message: "No JSON-LD" }];
      let missing = 0;
      blocks.each((_, el) => {
        try {
          const d = JSON.parse(page.$(el).contents().text());
          if (!JSON.stringify(d).includes('"@context"'))
            missing++;
        } catch {
        }
      });
      return [
        missing === 0 ? { status: "pass", message: "All JSON-LD blocks declare @context" } : { status: "warn", message: `${missing} JSON-LD block(s) missing @context` }
      ];
    }
  },
  {
    ...
    run({ page }) {
      ...
      return [
        /"sameAs"\s*:/.test(page.html) ? { status: "pass", message: "Organization declares sameAs" } : { status: "warn", message: "Organization schema has no sameAs links" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      if (!/"@type"\s*:\s*"(Organization|LocalBusiness)"/.test(page.html))
        return [{ status: "pass", message: "No Organization schema" }];
      return [
        /"logo"\s*:/.test(page.html) ? { status: "pass", message: "Organization declares a logo" } : { status: "warn", message: "Organization schema has no logo" }
      ];
    }
  },
  {
  ...
  },
  {
    ...
    run({ page }) {
      if (!/"@type"\s*:\s*"(Article|NewsArticle|BlogPosting)"/.test(page.html))
        return [{ status: "pass", message: "No article schema" }];
      return [
        /"image"\s*:/.test(page.html) ? { status: "pass", message: "Article schema includes image" } : { status: "warn", message: "Article schema missing image" }
      ];
    }
  },
  {
  ...
  },
  {
    ...
    run({ page }) {
      const blocks = page.$('script[type="application/ld+json"]');
      if (blocks.length === 0)
        return [{ status: "pass", message: "No JSON-LD to validate" }];
      let bad = 0;
      blocks.each((_, el) => {
        try {
      ...
      return [
        bad === 0 ? { status: "pass", message: "All JSON-LD blocks parse" } : { status: "fail", message: `${bad} JSON-LD block(s) invalid` }
      ];
    }
  },
  {
    ...
    run({ page }) {
      ...
      blocks.each((_, el) => {
        try {
          const d = JSON.parse(page.$(el).contents().text());
          if (JSON.stringify(d).includes('"@type"'))
            typed++;
        } catch {
        }
      });
      return [
        typed > 0 ? { status: "pass", message: "JSON-LD declares @type" } : { status: "warn", message: "No @type in JSON-LD" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      return [
        page.html.includes("BreadcrumbList") ? { status: "pass", message: "BreadcrumbList present" } : { status: "info", message: "No breadcrumb schema" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      ...
      page.$('script[type="application/ld+json"]').each((_, el) => {
        try {
          const d = JSON.parse(page.$(el).contents().text());
          const t = Array.isArray(d) ? d.map((x) => x["@type"]) : [d["@type"]];
      ...
      return [
        dup.length === 0 ? { status: "pass", message: "No duplicate schema types" } : { status: "info", message: `Duplicate schema type(s): ${[...new Set(dup)].join(", ")}` }
      ];
    }
  }
];

// node_modules/safi-studio-scanner/dist/rules/security.js
var securityRules = [
  {
    ...
    run({ page }) {
      ...
      return [
        s && /\d/.test(s) ? { status: "warn", message: `Server header reveals a version: ${s}`, evidence: s } : { status: "pass", message: "Server header does not leak a version" }
      ];
    }
  },
  {
  ...
  },
  {
    ...
    run({ page }) {
      ...
      if (!c)
        return [{ status: "pass", message: "No cookies set on this response" }];
      ...
      return [
        missing.length === 0 ? { status: "pass", message: "Cookies set with Secure and HttpOnly" } : { status: "warn", message: `Cookie missing ${missing.join(" and ")}` }
      ];
    }
  },
  {
    ...
    run({ page }) {
      return [
        page.headers["cross-origin-opener-policy"] ? { status: "pass", message: "COOP set" } : { status: "info", message: "No Cross-Origin-Opener-Policy header" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      return [
        page.finalUrl.startsWith("https://") ? { status: "pass", message: "HTTPS" } : { status: "fail", message: "Not served over HTTPS" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      return [
        page.headers["strict-transport-security"] ? { status: "pass", message: "HSTS set" } : { status: "warn", message: "Missing Strict-Transport-Security" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      return [
        page.headers["content-security-policy"] ? { status: "pass", message: "CSP set" } : { status: "warn", message: "Missing Content-Security-Policy" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      ...
      return [
        ok ? { status: "pass", message: "Clickjacking protection set" } : { status: "warn", message: "No X-Frame-Options or frame-ancestors" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      return [
        (page.headers["x-content-type-options"] || "").toLowerCase() === "nosniff" ? { status: "pass", message: "nosniff set" } : { status: "warn", message: "Missing X-Content-Type-Options: nosniff" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      return [
        page.headers["referrer-policy"] ? { status: "pass", message: "Referrer-Policy set" } : { status: "warn", message: "Missing Referrer-Policy" }
      ];
    }
  },
  {
  ...
  },
  {
    ...
    run({ page }) {
      return [
        page.headers["permissions-policy"] ? { status: "pass", message: "Permissions-Policy set" } : { status: "info", message: "No Permissions-Policy header" }
      ];
    }
  }
];

// node_modules/safi-studio-scanner/dist/rules/crawlability.js
var crawlabilityRules = [
  {
    ...
    run({ site }) {
      return [
        site.robots.exists ? { status: "pass", message: "robots.txt found" } : { status: "warn", message: "No robots.txt" }
      ];
    }
  },
  {
    ...
    run({ site }) {
      if (!site.robots.exists)
        return [{ status: "pass", message: "No robots.txt to validate" }];
      const ok = /(user-agent:|disallow:|allow:|sitemap:)/i.test(site.robots.content);
      return [
        ok ? { status: "pass", message: "robots.txt has valid directives" } : { status: "warn", message: "robots.txt has no recognizable directives" }
      ];
    }
  },
  {
    ...
    run({ site }) {
      return [
        site.sitemap.exists ? { status: "pass", message: `Sitemap with ${site.sitemap.urls.length} URLs` } : { status: "warn", message: "No sitemap found" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      ...
      return [
        blocked ? { status: "fail", message: "Page is set to noindex" } : { status: "pass", message: "Page is indexable" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      const c = page.$('link[rel="canonical"]').attr("href")?.trim();
      if (!c)
        return [{ status: "warn", message: "No canonical to validate" }];
      try {
  ...
  },
  {
    ...
    run({ page }) {
      ...
      if (!x)
        return [{ status: "pass", message: "No restrictive X-Robots-Tag" }];
      return [
        x.includes("noindex") || x.includes("nofollow") ? { status: "warn", message: `X-Robots-Tag: ${x}` } : { status: "pass", message: `X-Robots-Tag: ${x}` }
      ];
    }
  }
];

// node_modules/safi-studio-scanner/dist/rules/url-structure.js
function parts(rawUrl) {
  try {
...
var urlStructureRules = [
  {
    ...
    run({ page }) {
      ...
      return [
        /[A-Z]/.test(url.pathname) ? { status: "warn", message: "URL path contains uppercase letters", evidence: url.pathname } : { status: "pass", message: "URL is lowercase" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      ...
      return [
        path.includes("_") ? { status: "warn", message: "URL uses underscores between words", evidence: path } : { status: "pass", message: "No underscores in URL" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      return [
        /%20|\s/.test(page.url.split("#")[0]) ? { status: "warn", message: "URL contains spaces or %20", evidence: page.url } : { status: "pass", message: "No spaces in URL" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      ...
      return [
        ...
        /[^\u0000-\u007F]/.test(raw) ? { status: "warn", message: "URL contains raw non-ASCII characters", evidence: raw } : { status: "pass", message: "URL uses ASCII or encoded characters" }
      ];
    }
  },
  {
  ...
  },
  {
    ...
    run({ page }) {
      ...
      const idLike = /^\d+$/.test(last2) || url.searchParams.has("id") || url.searchParams.has("p");
      if (segments.length === 0)
        return [{ status: "pass", message: "Home URL" }];
      return [
        idLike ? { status: "warn", message: "URL relies on a numeric ID rather than words", evidence: url.pathname + url.search } : { status: "pass", message: "URL uses readable words" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      ...
      if (len <= 100)
        return [{ status: "pass", message: `URL is ${len} characters` }];
      if (len <= 115)
        return [{ status: "warn", message: `Long URL (${len} characters)` }];
  ...
  },
  {
    ...
    run({ page }) {
      ...
      if (segments.length <= 4)
        return [{ status: "pass", message: `${segments.length} path segment(s)` }];
      return [{ status: "warn", message: `Deep path (${segments.length} segments)`, evidence: "/" + segments.join("/") }];
    }
  }
];

// node_modules/safi-studio-scanner/dist/rules/social-media.js
var SOCIAL_HOSTS = /(linkedin\.com|twitter\.com|x\.com|instagram\.com|facebook\.com|youtube\.com|github\.com|tiktok\.com)/i;
var socialMediaRules = [
  {
    ...
    run({ page }) {
      return [
        page.$('meta[name="twitter:card"]').attr("content") ? { status: "pass", message: "Twitter Card present" } : { status: "warn", message: "No twitter:card meta" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      ...
      if (!img)
        return [{ status: "warn", message: "No og:image to validate" }];
      return [
        /^https?:\/\//i.test(img) ? { status: "pass", message: "og:image is an absolute URL" } : { status: "fail", message: "og:image is not an absolute URL", evidence: img }
      ];
    }
  },
  {
    ...
    run({ page }) {
      ...
      if (!og || !canon)
        return [{ status: "pass", message: "og:url or canonical absent, nothing to compare" }];
      return [
        og.replace(/\/$/, "") === canon.replace(/\/$/, "") ? { status: "pass", message: "og:url matches canonical" } : { status: "warn", message: "og:url does not match canonical", evidence: `${og} vs ${canon}` }
      ];
    }
  },
  {
...
// node_modules/safi-studio-scanner/dist/rules/internationalization.js
var internationalizationRules = [
  {
  ...
  },
  {
    ...
    run({ page }) {
      ...
      if (tags.length === 0)
        return [{ status: "pass", message: "No hreflang to validate" }];
      const valid = /^([a-z]{2,3}(-[A-Za-z]{2,4})?|x-default)$/;
      const bad = [];
      let hasDefault = false;
      tags.each((_, el) => {
        const v = page.$(el).attr("hreflang") || "";
        if (v.toLowerCase() === "x-default")
          hasDefault = true;
        if (!valid.test(v))
          bad.push(v);
      });
      if (bad.length)
        return [{ status: "warn", message: `Invalid hreflang value(s): ${bad.join(", ")}` }];
      if (!hasDefault)
        return [{ status: "warn", message: "hreflang set has no x-default" }];
...
var legalRules = [
  {
    ...
    run({ page }) {
      return [
        linkMatches(page.links, /privacy/i) ? { status: "pass", message: "Privacy policy link found" } : { status: "warn", message: "No privacy policy link found" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      return [
        linkMatches(page.links, /terms|conditions/i) ? { status: "pass", message: "Terms link found" } : { status: "info", message: "No terms of service link found" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      const hint = /cookieconsent|cookiebot|onetrust|osano|termly|usercentrics|cookie-?banner|cookie consent/i.test(page.html);
      return [
        hint ? { status: "pass", message: "Cookie consent mechanism detected" } : { status: "info", message: "No cookie consent banner detected" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      return [
        linkMatches(page.links, /contact|mailto:/i) ? { status: "pass", message: "Contact link found" } : { status: "info", message: "No contact link found" }
      ];
    }
  }
];

// node_modules/safi-studio-scanner/dist/rules/analytics.js
var ANALYTICS_HINTS = /(googletagmanager\.com|gtag\(|google-analytics\.com|ga\.js|analytics\.js|plausible\.io|cdn\.usefathom|matomo|segment\.com\/analytics|posthog|mixpanel|hotjar|clarity\.ms)/i;
var analyticsRules = [
  {
    ...
    run({ page }) {
      return [
        ANALYTICS_HINTS.test(page.html) ? { status: "pass", message: "Analytics tag detected" } : { status: "warn", message: "No analytics tag detected" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      ...
      if (!usesGoogle)
        return [{ status: "pass", message: "No Google tag, consent mode not required" }];
      return [
        /consent['"\s,]+(default|update)/i.test(page.html) ? { status: "pass", message: "Google Consent Mode configured" } : { status: "info", message: "Google tag present without visible Consent Mode" }
      ];
    }
  }
];

// node_modules/safi-studio-scanner/dist/rules/eeat.js
var eeatRules = [
  {
    id: "eeat/author",
    category: "eeat",
    ...
    run({ page }) {
      const has2 = page.$('meta[name="author"]').attr("content") || page.$('[rel="author"]').length > 0 || /"author"\s*:/.test(page.html);
      return [
        has2 ? { status: "pass", message: "Author attribution present" } : { status: "warn", message: "No author attribution found" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      const has2 = page.$('meta[property="article:published_time"]').length > 0 || page.$('meta[property="article:modified_time"]').length > 0 || page.$("time[datetime]").length > 0 || /"date(Published|Modified)"\s*:/.test(page.html);
      return [
        has2 ? { status: "pass", message: "Date information present" } : { status: "info", message: "No published or updated date found" }
      ];
    }
  },
  {
...
// node_modules/safi-studio-scanner/dist/rules/accessibility.js
var IMPACT_SEVERITY = {
  critical: "error",
  serious: "error",
  moderate: "warning",
...
// node_modules/safi-studio-scanner/dist/rules/performance.js
function withMetrics(page, fn) {
  if (!page.browser || !page.browser.ok)
    return [{ status: "warn", message: "Page could not be rendered for performance checks" }];
  const m = page.browser.metrics;
  if (!m)
    return [{ status: "info", message: "Not measured (no performance provider)" }];
  return [fn(m)];
}
var performanceRules = [
  {
    ...
    run({ page }) {
      ...
      page.$("head script[src]").each((_, el) => {
        const s = page.$(el);
        if (!s.attr("defer") && !s.attr("async") && (s.attr("type") || "") !== "module")
          blocking++;
      });
      return [
        blocking === 0 ? { status: "pass", message: "No render-blocking scripts in head" } : { status: "warn", message: `${blocking} render-blocking script(s) in head` }
      ];
    }
  },
  {
  ...
  },
  {
    ...
    run({ page }) {
      ...
      return [
        bytes <= 3e4 ? { status: "pass", message: `${(bytes / 1e3).toFixed(1)}KB inline JS` } : { status: "warn", message: `${(bytes / 1e3).toFixed(1)}KB of inline JavaScript` }
      ];
    }
  },
  {
    ...
    run({ page }) {
      ...
      return [
        /gzip|br|deflate|zstd/.test(enc) ? { status: "pass", message: `Compressed (${enc})` } : { status: "warn", message: "HTML response is not compressed" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      return [
        page.headers["cache-control"] ? { status: "pass", message: `Cache-Control: ${page.headers["cache-control"]}` } : { status: "warn", message: "No Cache-Control header" }
      ];
    }
  },
  {
    ...
    run({ page }) {
      const kb = page.html.length / 1e3;
      if (kb <= 100)
        return [{ status: "pass", message: `${kb.toFixed(0)}KB HTML` }];
      if (kb <= 200)
        return [{ status: "warn", message: `Large HTML (${kb.toFixed(0)}KB)` }];
  ...
  },
  {
    ...
    run({ page }) {
      return withMetrics(page, (m) => {
        if (m.lcpMs === 0)
          return { status: "info", message: "LCP not measured" };
        if (m.lcpMs <= 2500)
          return { status: "pass", message: `LCP ${(m.lcpMs / 1e3).toFixed(2)}s` };
        if (m.lcpMs <= 4e3)
          return { status: "warn", message: `LCP needs work (${(m.lcpMs / 1e3).toFixed(2)}s)` };
        return { status: "fail", message: `Poor LCP (${(m.lcpMs / 1e3).toFixed(2)}s)` };
      });
    }
  },
  {
    ...
    run({ page }) {
      return withMetrics(page, (m) => {
        if (m.cls <= 0.1)
          return { status: "pass", message: `CLS ${m.cls}` };
        if (m.cls <= 0.25)
          return { status: "warn", message: `CLS needs work (${m.cls})` };
  ...
  },
  {
    ...
    run({ page }) {
      return withMetrics(page, (m) => {
        if (m.ttfbMs <= 800)
          return { status: "pass", message: `TTFB ${m.ttfbMs}ms` };
        if (m.ttfbMs <= 1800)
          return { status: "warn", message: `Slow TTFB (${m.ttfbMs}ms)` };
  ...
  },
  {
    ...
    run({ page }) {
      return withMetrics(page, (m) => {
        if (m.domNodes <= 1500)
          return { status: "pass", message: `${m.domNodes} DOM nodes` };
        if (m.domNodes <= 3e3)
          return { status: "warn", message: `Large DOM (${m.domNodes} nodes)` };
  ...
  },
  {
    ...
    run({ page }) {
      return withMetrics(page, (m) => {
        const mb = m.transferBytes / 1e6;
        if (mb <= 3)
          return { status: "pass", message: `${mb.toFixed(2)}MB transferred` };
        if (mb <= 6)
          return { status: "warn", message: `Heavy page (${mb.toFixed(2)}MB)` };
  ...
  },
  {
    ...
    run({ page }) {
      return withMetrics(page, (m) => {
        if (m.requests <= 80)
          return { status: "pass", message: `${m.requests} requests` };
        if (m.requests <= 150)
          return { status: "warn", message: `Many requests (${m.requests})` };
...
// node_modules/safi-studio-scanner/dist/rules/index.js
var allRules = [
  ...coreSeoRules,
  ...contentRules,
  ...linksRules,
...
// node_modules/safi-studio-scanner/dist/audit.js
var DEFAULTS = {
  format: "html",
  timeout: 15e3,
...
async function audit(startUrl, options = {}) {
...
// node_modules/safi-studio-scanner/dist/report.js
function render3(report, format) {
  if (format === "json")
    ...
    for (const issues of g.pages.values())
      for (const f of issues)
        if (STATUS_RANK[f.status] < STATUS_RANK[worst])
          worst = f.status;
    const affected = [...g.pages.entries()].map(([url, issues]) => ({ url, issues: issues.filter((i) => i.status !== "pass") })).filter((x) => x.issues.length > 0);
    const group = {
      ruleId,
      title: g.title,
      category: g.category,
...
function auditFileName(report) {
  ...
  const stamp = isNaN(d.getTime()) ? "audit" : d.toISOString().slice(0, 16).replace(/[T:]/g, "-");
...
function listAudits(app, settings) {
  const folder = app.vault.getAbstractFileByPath((0, import_obsidian2.normalizePath)(settings.auditFolder));
  if (!(folder instanceof import_obsidian2.TFolder)) return [];
  const items = [];
  for (const child2 of folder.children) {
    if (!(child2 instanceof import_obsidian2.TFile) || child2.extension !== "md") continue;
    const fm = app.metadataCache.getFileCache(child2)?.frontmatter;
...
async function saveAudit(app, settings, report) {
  await ensureFolder(app, settings.auditFolder);
  const base = auditFileName(report).replace(/\.md$/, "");
  let path = (0, import_obsidian2.normalizePath)(`${settings.auditFolder}/${base}.md`);
  let i = 2;
  while (app.vault.getAbstractFileByPath(path)) {
    path = (0, import_obsidian2.normalizePath)(`${settings.auditFolder}/${base}-${i}.md`);
    i++;
  }
  return app.vault.create(path, buildNote(report));
}
async function openInObsidian(app, file) {
...
async function shimFetch(input, init2 = {}) {
